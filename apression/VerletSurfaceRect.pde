...
class VerletSurfaceRect extends VerletSurface {
      ...
      for (int j=0; j<cols; j++) {
        ...
        if (i<rows-1) {
          ...
          if (i==0 && j==0 && isFixedIndex[0]) {
            vSticks[stickCounter++] = new VerletStick(vBalls2D[i][j], vBalls2D[i+1][j], tension, new Tuple2D(0.0, 1.0));
          } 
          ...
          // anchor bottom right corner:  | stick
            ...
            vSticks[stickCounter++] = new VerletStick(vBalls2D[i][j], vBalls2D[i+1][j], tension, new Tuple2D(1.0, 0.0));
          } 
        ...
        if (j<cols-1) {
          // anchor top left corner:  -- stick
          if (i==0 && j==0 && isFixedIndex[0]) {
            vSticks[stickCounter++] = new VerletStick(vBalls2D[i][j], vBalls2D[i][j+1], tension, new Tuple2D(0.0, 1.0));
          }  // anchor top right corner:  -- stick
          else if (i==0 && j==cols-2 && isFixedIndex[1]) {
            vSticks[stickCounter++] = new VerletStick(vBalls2D[i][j], vBalls2D[i][j+1], tension, new Tuple2D(1.0, 0.0));
          }  // anchor bottom right corner:  -- stick
  ...
  void imgReset(int id) {
    ...
    switch(id) {
    case 0:
      img = loadImage("fish2.jpg");
      break;
    case 1:
      img = loadImage("ears.jpg");
      break;
    case 2:
      img = loadImage("silly_girl.jpg");
      break;
    case 3:
    ...
    case 4:
      ...
      break;
    case 5:
      img = loadImage("l_elliot.jpg");
      break;
    case 6:
      img = loadImage("antique_doll.jpg");
      break;
    case 7:
      img = loadImage("cyborg.jpg");
      break;
    case 8:
      img = loadImage("gas_mask.jpg");
      break;
    case 9:
      img = loadImage("larva.jpg");
      break;
    }
  }
}
