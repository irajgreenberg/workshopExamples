class VerletStick {
  ...
  void constrainLen(){
    ...
    b1.pos.x += delta.x * (0.5 * stiffness * difference);
    b1.pos.y += delta.y * (0.5 * stiffness * difference);
    b2.pos.x -= delta.x * (0.5 * stiffness * difference);
    b2.pos.y -= delta.y * (0.5 * stiffness * difference);
  }
