...
class Emitter {
  ...
  Emitter(PVector loc, float sketchFrameRate, PVector birthPath, 
  float sprayWidth, Particle[] p){
    this.loc = loc;
  ...
  // constructor for single emission with birthRate param 
  // (good for explosions, etc)
  Emitter(PVector loc, PVector birthPath, float birthRate, 
  float sprayWidth, Particle[] p){
    this.loc = loc;
  ...
  // called at the beginning of each emission cyle 
  // (and initially by the constructor)
  void init(int i){
  ...
  void emit(){
    for (int i=0; i<particleCounter; i++){
      ...
      if (lifeTime[i] < p[i].lifeSpan){
        ...
        p[i].vel.y += random(-environment.turbulence, 
        environment.turbulence) + environment.wind.y;
        p[i].vel.x += random(-environment.turbulence, 
        environment.turbulence) + environment.wind.x;
